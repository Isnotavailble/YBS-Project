...
function BusStop(props) {
    ...
    useEffect(() => {
        if (relatedAllBus !== null) {
            ...
            // set default bus line which is "loaded" state when every input from search bar is done 
            // to avoid previous data collapsing
            const defaultBusLine = () => {
                let temp = [];
                relatedAllBus[0].bus[0].destinations.forEach(stop => {
                    temp.push([[stop.latitude, stop.longitude], stop.name]);
                })
                return temp;
            }
            setSelectedBus(relatedAllBus[0].bus[0].num);
            setLoaded(defaultBusLine());
            setSelectedStop(defaultStop);
            console.log("set Default Bus is Done!", defaultStop);
                    ...
                    {relatedAllBus.map((busStop, index) =>
                        <div key={index} id="bus-and-stop" >
                            <div className="bus-stop-name">
                                ...
                                <button onClick={() => {
                                    console.log("selected : ", busStop.location);
                                    setSelectedStop(busStop.location); setSidePage("BusStopMap"); setError(false);
                                }}>See BusStop</button>
                            </div>
